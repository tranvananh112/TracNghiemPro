class QuizManager {
    constructor() {
        // Load quizzes từ localStorage trước, nếu không có thì từ sessionStorage
        let storedQuizzes = null;
        
        try {
            storedQuizzes = localStorage.getItem('quizzes');
            if (storedQuizzes) {
                this.quizzes = JSON.parse(storedQuizzes);
                console.log('✅ Đã load quiz từ localStorage:', this.quizzes.length, 'quiz');
            }
        } catch (error) {
            console.warn('⚠️ Lỗi khi load từ localStorage:', error);
        }
        
        // Nếu localStorage không có hoặc lỗi, thử sessionStorage
        if (!this.quizzes || this.quizzes.length === 0) {
            try {
                const sessionQuizzes = sessionStorage.getItem('quizzes');
                if (sessionQuizzes) {
                    this.quizzes = JSON.parse(sessionQuizzes);
                    console.log('✅ Đã load quiz từ sessionStorage:', this.quizzes.length, 'quiz');
                }
            } catch (error) {
                console.warn('⚠️ Lỗi khi load từ sessionStorage:', error);
            }
        }
        
        // Nếu cả hai đều không có, khởi tạo mảng rỗng
        if (!this.quizzes) {
            this.quizzes = [];
            console.log('📝 Khởi tạo danh sách quiz rỗng');
        }
        ...
        console.log('🎯 QuizManager đã khởi tạo thành công');
    }
    ...
    initializeEventListeners() {
        ...
        // Tab navigation - Support both .nav-btn and .nav-item
        ...
        if (processQuizBtn) {
            processQuizBtn.addEventListener('click', () => {
                console.log('🔘 Nút tạo quiz được click');
                this.processQuiz();
            });
        } else {
            console.error('❌ Không tìm thấy nút process-quiz');
        }
    ...
    switchTab(tabName) {
        ...
        // Update nav buttons - Support both .nav-btn and .nav-item
        document.querySelectorAll('.nav-btn, .nav-item').forEach(btn => {
        ...
        if (tabName === 'manage') {
            ...
            this.updateQuizSelector();
        } else if (tabName === 'my-library' && window.personalMenuManager) {
            window.personalMenuManager.renderMyLibrary();
        } else if (tabName === 'recent-access' && window.personalMenuManager) {
            window.personalMenuManager.renderRecentAccess();
        } else if (tabName === 'favorites' && window.personalMenuManager) {
            window.personalMenuManager.renderFavorites();
        } else if (tabName === 'my-results' && window.personalMenuManager) {
            window.personalMenuManager.renderMyResults();
        }
    }

    processQuiz() {
        console.log('🚀 Bắt đầu xử lý tạo quiz...');
        ...
        console.log('📝 Dữ liệu đầu vào:', {
            ...
            questionsLength: questionsText.length,
            answersLength: answersText.length
        });

        // Validation chi tiết
        if (!title) {
            this.showToast('⚠️ Vui lòng nhập tên bài quiz!', 'error');
            document.getElementById('quiz-title').focus();
            return;
        }

        if (title.length < 3) {
            this.showToast('⚠️ Tên bài quiz phải có ít nhất 3 ký tự!', 'error');
            document.getElementById('quiz-title').focus();
            return;
        }

        if (!questionsText) {
            this.showToast('⚠️ Vui lòng nhập câu hỏi!', 'error');
            document.getElementById('questions-input').focus();
            return;
        }

        if (!answersText) {
            this.showToast('⚠️ Vui lòng nhập đáp án!', 'error');
            document.getElementById('answers-input').focus();
            return;
        }

        if (questionsText.length < 50) {
            this.showToast('⚠️ Nội dung câu hỏi quá ngắn! Vui lòng nhập đầy đủ hơn.', 'error');
            document.getElementById('questions-input').focus();
            return;
        }

        if (answersText.length < 10) {
            this.showToast('⚠️ Nội dung đáp án quá ngắn! Vui lòng nhập đầy đủ hơn.', 'error');
            document.getElementById('answers-input').focus();
            return;
        }
        ...
        try {
            ...
            console.log('✅ Phân tích câu hỏi thành công:', questions.length, 'câu');

            console.log('🔍 Đang phân tích đáp án...');
            const answers = this.parseAnswers(answersText, questions.length);
            console.log('✅ Phân tích đáp án thành công:', answers.length, 'đáp án');

            if (questions.length === 0) {
                this.showToast('❌ Không tìm thấy câu hỏi hợp lệ! Vui lòng kiểm tra định dạng.', 'error');
                return;
            }

            if (answers.length === 0) {
                this.showToast('❌ Không tìm thấy đáp án hợp lệ! Vui lòng kiểm tra định dạng.', 'error');
                return;
            }
            ...
            // Kiểm tra từng câu hỏi có đủ đáp án không
            for (let i = 0; i < questions.length; i++) {
                if (!questions[i].options || questions[i].options.length < 2) {
                    this.showToast(`❌ Câu ${i + 1} không có đủ lựa chọn! Cần ít nhất 2 đáp án.`, 'error');
                    return;
                }
            }
            ...
            console.log('💾 Đang lưu quiz...', quiz);
            ...
            console.log('📋 Thông tin debug:', {
                questionsText: questionsText.substring(0, 200) + '...',
                answersText: answersText.substring(0, 200) + '...',
                error: error.stack
            });
        }
    }
    ...
    parseQuestions(text) {
        console.log('🔍 Bắt đầu phân tích câu hỏi...');
        const questions = [];
        const lines = text.split('\n').map(line => line.trim()).filter(line => line);
        
        console.log('📄 Số dòng đã lọc:', lines.length);
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.match(/^Câu\s+\d+[:：.]/) || 
                ...
                if (currentQuestion && currentOptions.length >= 2) {
                    const questionText = currentQuestion.replace(/^(Câu\s+\d+|Q\d+|Question\s+\d+|\d+)[:：.]\s*/i, '');
                    questions.push({
                        question: questionText,
                        options: currentOptions
                    });
                    console.log(`✅ Đã lưu câu hỏi: "${questionText.substring(0, 50)}..." với ${currentOptions.length} đáp án`);
                }
                
                // Start new question
                currentQuestion = line;
                currentOptions = [];
                console.log(`🆕 Bắt đầu câu hỏi mới: "${line}"`);
                
            } else if (line.match(/^[A-Da-d][:：.]\s*/) || 
                       line.match(/^[A-Da-d]\s+/) ||
                       line.match(/^[A-Da-d]\.\s*/)) {
                
                // Extract option - hỗ trợ nhiều format
                ...
                if (option.length > 0) {
                    ...
                    console.log(`📋 Đáp án ${letter}: "${option}"`);
                }
            } else if (currentQuestion && line.length > 5) {
                // Nếu dòng không phải câu hỏi mới và không phải đáp án, có thể là phần mở rộng của câu hỏi
                console.log(`📝 Có thể là phần mở rộng câu hỏi: "${line}"`);
            }
        }
        ...
        if (currentQuestion && currentOptions.length >= 2) {
            ...
            console.log(`✅ Đã lưu câu hỏi cuối: "${questionText.substring(0, 50)}..." với ${currentOptions.length} đáp án`);
        }

        console.log(`📊 Tổng kết: Tìm thấy ${questions.length} câu hỏi hợp lệ`);
    ...
    parseAnswers(text, expectedCount) {
        console.log('🔍 Bắt đầu phân tích đáp án...');
        const answers = [];
        const lines = text.split('\n').map(line => line.trim()).filter(line => line);
        
        console.log('📄 Số dòng đáp án:', lines.length);
        
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            console.log(`📝 Dòng đáp án ${i + 1}: "${line}"`);
            ...
            if (line.match(/^Câu\s+\d+\s*[:：.]\s*[A-Da-d]$/i)) {
                ...
                answers.push(answer);
                console.log(`✅ Đáp án từ format "Câu X: Y": ${answer}`);
            }
            // Format: "1. B" or "1: B" or "1 : B" (with spaces)
            else if (line.match(/^\d+\s*[:：.]\s*[A-Da-d]$/)) {
                const answer = line.replace(/^\d+\s*[:：.]\s*/, '').trim().toUpperCase();
                answers.push(answer);
                console.log(`✅ Đáp án từ format "X: Y": ${answer}`);
            }
            // Format: Just "B" or " B " (with spaces)
            else if (line.match(/^[A-Da-d]$/i)) {
                answers.push(line.toUpperCase());
                console.log(`✅ Đáp án đơn: ${line.toUpperCase()}`);
            }
            // Format: "A. B" hoặc "A: B" (nếu có nội dung sau chữ cái)
            else if (line.match(/^[A-Da-d]\s*[:：.]\s*[A-Da-d]$/i)) {
                const answer = line.charAt(0).toUpperCase();
                answers.push(answer);
                console.log(`✅ Đáp án từ format "X: Y": ${answer}`);
            }
            else {
                console.log(`⚠️ Dòng không hợp lệ: "${line}"`);
            }
        }

        console.log(`📊 Tổng kết: Tìm thấy ${answers.length} đáp án hợp lệ`);
        ...
        if (answers.length !== expectedCount) {
            throw new Error(`Cần ${expectedCount} đáp án, chỉ tìm thấy ${answers.length}! Vui lòng kiểm tra lại.\n\n` +
                          `Đã tìm thấy: ${answers.join(', ')}\n` +
                          `Cần: ${expectedCount} đáp án`);
        }
    ...
    saveQuizzes() {
        try {
            ...
            console.log('✅ Đã lưu quiz vào localStorage thành công');
        } catch (error) {
            console.error('❌ Lỗi khi lưu vào localStorage:', error);
            this.showToast('❌ Lỗi khi lưu quiz! Có thể do trình duyệt chặn localStorage.', 'error');
            
            // Thử lưu vào sessionStorage làm backup
            try {
                sessionStorage.setItem('quizzes', JSON.stringify(this.quizzes));
                console.log('✅ Đã lưu backup vào sessionStorage');
                this.showToast('⚠️ Đã lưu vào bộ nhớ tạm. Dữ liệu sẽ mất khi đóng trình duyệt.', 'warning');
            } catch (sessionError) {
                console.error('❌ Cả localStorage và sessionStorage đều lỗi:', sessionError);
                this.showToast('❌ Không thể lưu quiz! Vui lòng kiểm tra cài đặt trình duyệt.', 'error');
            }
        }
    }
