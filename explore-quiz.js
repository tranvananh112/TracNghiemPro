...
class ExploreQuizManager {
    ...
    async initialize() {
        ...
        // Nếu Supabase không khả dụng, kiểm tra Local Server (im lặng)
        if (!this.isSupabaseAvailable) {
            // Chỉ kiểm tra server nếu người dùng thực sự cần
            // Không tự động kiểm tra để tránh lỗi console
            this.isServerOnline = false;
    ...
    async checkServerStatus() {
        try {
            // ⭐ KIỂM TRA SUPABASE TRƯỚC
            if (window.supabaseQuizManager && window.supabaseQuizManager.isAvailable()) {
                console.log('✅ Using Supabase instead of localhost server');
                this.isServerOnline = true;
                this.offlineMode = false;
                this.updateServerStatus(true);
                return true;
            ...
            // ⚠️ KHÔNG KẾT NỐI LOCALHOST NẾU KHÔNG CẦN
            if (this.API_BASE_URL.includes('localhost')) {
                console.log('⚠️ Localhost server disabled - Use Supabase or offline mode');
                this.isServerOnline = false;
                this.offlineMode = true;
            ...
            // Không log lỗi kết nối vì đây là bình thường khi chưa chạy server
            this.isServerOnline = false;
            this.offlineMode = true;
            this.updateServerStatus(false);
        }
    ...
    updateServerStatus(isOnline) {
        ...
        if (statusIndicator) {
            if (isOnline) {
                ...
                // Không hiển thị status offline nữa
                statusIndicator.innerHTML = '';
    ...
    showOfflineEmptyState() {
        ...
        if (container) {
            // Không hiển thị gì cả - đã tắt hoàn toàn thông báo offline
            container.innerHTML = '';
