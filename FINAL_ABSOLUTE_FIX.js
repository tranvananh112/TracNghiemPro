...
(function() {
    ...
    if (window.fetch) {
        ...
        window.fetch = function(...args) {
            ...
            // Bắt lỗi kết nối và không log ra console
            return originalFetch.apply(this, args).catch(error => {
                // Chỉ log lỗi quan trọng, bỏ qua lỗi kết nối server
                if (!url.includes('localhost:3000') && 
                    !url.includes('shared-quizzes') &&
                    !error.message.includes('Failed to fetch')) {
                    console.warn('Fetch error:', error.message);
                }
                // Throw lại lỗi để code gọi xử lý
                throw error;
            });
        };
            ...
            window.QuizManager.prototype.submitQuiz = function() {
                ...
                setTimeout(async () => {
                    ...
                    if (quiz && quiz.isRoomQuiz && quiz.roomId && quiz.userName && results) {
                        ...
                        // ⭐ Kiểm tra xem phòng là cloud hay offline
                        ...
                        // ☁️ LƯU LÊN CLOUD (nếu có Supabase và phòng cloud)
                        if (isCloudRoom && hasSupabase) {
                            try {
                                ...
                                if (window.notify) {
                                    window.notify.success('Đã lưu kết quả!', 'Kết quả đã được lưu lên cloud', 4000);
                                } else {
                                    alert('✅ Đã lưu kết quả lên cloud!');
                                }
                                
                            } catch (error) {
                                console.error('❌ Cloud save error:', error);
                                // Fallback to offline
                                console.log('⚠️ Falling back to offline save...');
                                saveResultOffline(quiz.roomId, entry);
                            }
                        } 
                        // 💾 LƯU OFFLINE (phòng offline hoặc không có Supabase)
                        else {
                            console.log('💾 Saving to OFFLINE mode...');
                            saveResultOffline(quiz.roomId, entry);
                        }
                    }
                }, 2000);
                
                // ⭐ HÀM LƯU KẾT QUẢ OFFLINE
                function saveResultOffline(roomId, entry) {
                    try {
                        // Lấy danh sách phòng offline
                        const offlineRooms = JSON.parse(localStorage.getItem('offlineRooms') || '[]');
                        const roomIndex = offlineRooms.findIndex(r => r.id === roomId);
                        
                        if (roomIndex === -1) {
                            console.warn('⚠️ Room not found in offline storage');
                            if (window.notify) {
                                window.notify.warning('Phòng không tồn tại', 'Không tìm thấy phòng trong bộ nhớ', 4000);
                            }
                            return;
                        }
                        ...
                        room.participants = uniqueUsers.size;
                        ...
                        if (window.notify) {
                            window.notify.success('Đã lưu kết quả!', 'Kết quả đã được lưu offline', 4000);
                        } else {
                            alert('✅ Đã lưu kết quả offline!');
                        }
                        
                    } catch (error) {
                        console.error('❌ Offline save error:', error);
                        if (window.notify) {
                            window.notify.error('Lỗi lưu kết quả', error.message, 5000);
                        } else {
